...
export default function JobDescriptionBoard() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsCopied(false); // Reset copied state on fresh generation
    ...
    try {
      ...
      // Guard statement to safely catch malformed objects or API errors
      if (data && data.tweet) {
        setTweet(data.tweet.trim());
      } else if (data && data.error) {
        setTweet(`⚠️ Backend API Error: ${data.error.message || JSON.stringify(data.error)}\n\nPlease verify your Groq key configurations in .env.local.`);
      } else {
        setTweet("⚠️ Received an unexpected payload format. Double check your src/pages/api/returnJobDescription.js handler framework.");
      }
    } catch (err) {
      console.error("Frontend Fetch Error Loop:", err);
      setTweet("❌ Connection refused. Make sure your local Next.js development server is active ('npm run dev').");
    } finally {
  ...
  const handleCopy = () => {
    ...
    setIsCopied(true);
    setTimeout(() => setIsCopied(false), 2000); // Resets button text back to default after 2 seconds
  };
